...
=head2 Wie ist es im Vergleich zu ASP, PHP, ePerl?

Embperl ist nicht der einzige Prozessor für eingefügten Perlcode.
ASP zusammen mit ActivesStates Perldistribution erlaubt dies für
den Microsoft IIS, ePerl ist eine Lösung, die für alle Arten von
ASCII Dateien gut funktioniert und es gibt weitere
Perlmodule die diesen Zweck erfüllen. PHP ist eine sehr bekannte
Lösung um einfach Webseiten, mit eingebetteten Code und Datenbankanbindung,
zu erstellen, man muß jedoch dessen eigene Syntax lernen.

=head2 Zusätzliche HTML Features

Der Hauptvorteil von Embperl ist, daß es speziell auf HTML
