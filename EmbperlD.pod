...
C<embpexec.pl> (bzw. C<embpexec.bat> unter Windows) in Ihr CGI
Verzeichnis kopieren (oft C</cgi-bin>). Zum Aufrufen verwenden Sie
die URL

 http://www.domain.de/cgi-bin/embpexec.pl/uri/ihres/embperl/dokuments

Die extra Pfadinformationen die C<embpexec.pl> übergeben werden, geben
dabei den Ort des Embperl Dokuments an.

Unter Apache kann C<embpexec.pl> auch als Handler definiert werden:
...
C<%fdat> Element direkt enthalten, die uploadInfos erschließen sich, wenn das Element
zweimal dereferenziert wird.
...
Um das Session Management zu aktivieren müssen Sie I<Apache::Session> vor
I<Embperl> laden. Außerdem müssen Sie I<Embperl> mitteilen, welcher
Speicher und Lockingmechanismus genutzt werden soll, ggf. müssen Sie
auch weitere Argument für I<Apache::Session> setzen.
...
dass selbe direkt in die F<httpd.conf> eingetragen, sieht so aus:
...
Ältere Versionen von I<Embperl> (bis einschließlich 1.2b6) arbeiten nur
mit Apache::Session 0.17 zusammen, die Konfiguration dafür sieht
folgendermaßen aus:
...
oder tragen Sie folgende Zeilen in httpd.conf ein:
...
Verwenden Sie nach Möglichkeit nur noch Apache::Session 1.01 oder höher!
Müssen Sie noch auf I<Apache::Session> 0.17 arbeiten und wollen I<DBI>
als Speicher einsetzen, so verwenden Sie I<Apache::Session::epDBI> statt
I<Apache::Session::DBI>. Bitte beachten Sie das I<Apache::Session>
0.17 und 1.01 nicht kompatibel zueinander sind und eine andere
...
B<HINWEIS:> Session Handling pro Benutzer funktioniert z.Z. nur unter
I<mod_perl>.


B<WICHTIGER HINWEIS für DBI und %mdat Gebrauch:> Apache::Session 0.17
ist nicht in der Lage Zugriffe korrekt zu serialisieren, verwenden Sie
deshalb I<Apache::Session> 1.01 oder höher (wird erst ab I<Embperl> 1.2b7
unterstützt.
...
B<Hinweis 1:> C<exit> steht z.Z. nur unter I<mod_perl> zur Verfügung.
...
der mod_perl Mailingliste geführt. Sollten Sie Probleme mit I<Embperl>
haben, die sich nach der Lektüre dieser Dokumentation sowie der L<FAQs|"faq_pod">
...
majordomo@apache.org mit dem Text "subscribe modperl" als Text. 
