...
=head1 Breaking up your code in components
...
copying the source code to every page, you can include B<Embperl modules>
...
caple of including such partial pages, you can also pass arguments - 
for example, to tell the navigation bar which element to highlight.
...
a link, for every item in the array @buttons. Also the text which is equal
to the one passed as first parameter is displayed in bold.
There is also a long form of the Execute call, which allows you to control
all details how the called module is executed.
