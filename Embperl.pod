...
B<See also>: L<"perldoc EmbperlObject"|"EmbperlObject.pod"> to see how to setup I<Embperl> in a way to
...
alternativ (1.2.1 and up) you can return a reference to a hash with the elements C<mtime> and C<inputfile>
...
(see also L<$escmode>). Also when inside such a URL, I<Embperl> expands array and hash refernces
...
Same purpose as C<%CLEANUP>, but you may add filenames and all variables defined inside that file
will be cleaned up.
...
Additionaly C<EMBPERL_SESSION_CLASSES> can (optionaly) take two further classnames, which specifies
...
Vim Syntaxfile from Steve Willer could be found at http://www.interlog.com/~willer/embperl.vim

Vim Syntaxfile from Kee Hinckley could be found at http://www.somewhere.com/software/
